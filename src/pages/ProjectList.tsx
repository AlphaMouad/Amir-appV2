  ...
  return (
    ...
    >
      <motion.div variants={item} className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4 border-b border-white/5">
        <div>
          <h1 className="text-4xl font-outfit font-black tracking-tight text-white mb-2">Projects</h1>
          <p className="text-indigo-200/60 font-medium">Manage your elite portfolio</p>
        </div>
        <Button onClick={() => setIsCreating(!isCreating)} className="bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all px-6 py-5">
      ...
      {isCreating && (
        <motion.div 
          ...
          className="glass-card p-6 md:p-8 border border-indigo-500/30 shadow-[0_0_30px_rgba(79,70,229,0.1)] relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/10 blur-[100px] rounded-full pointer-events-none" />
          
          <h2 className="text-2xl font-outfit font-black mb-6 text-white relative z-10">Initialize Portfolio Asset</h2>
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
            <div>
              <label className="block text-xs font-bold text-indigo-300 uppercase tracking-widest mb-2">Asset Name</label>
              <input 
                type="text" 
                required
                className="w-full bg-black/40 backdrop-blur-md border border-white/10 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl p-3.5 text-sm text-white transition-all outline-none placeholder:text-slate-600 shadow-inner"
            ...
            <div>
              <label className="block text-xs font-bold text-indigo-300 uppercase tracking-widest mb-2">Client Identity</label>
              <input 
                type="text" 
                required
                className="w-full bg-black/40 backdrop-blur-md border border-white/10 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl p-3.5 text-sm text-white transition-all outline-none placeholder:text-slate-600 shadow-inner"
            ...
            <div>
              <label className="block text-xs font-bold text-indigo-300 uppercase tracking-widest mb-2">Contractor (Optional)</label>
              <input 
                type="text" 
                className="w-full bg-black/40 backdrop-blur-md border border-white/10 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl p-3.5 text-sm text-white transition-all outline-none placeholder:text-slate-600 shadow-inner"
            ...
            <div className="md:col-span-3 flex space-x-4 pt-4 border-t border-white/5 mt-2">
              <Button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl px-8 font-bold shadow-[0_0_15px_rgba(79,70,229,0.3)]">Deploy Asset</Button>
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col relative">
                <div className="flex items-start justify-between mb-8 relative z-10">
                  <div className="flex items-center space-x-4">
                    <div className="h-14 w-14 bg-gradient-to-br from-indigo-500/20 to-indigo-600/20 border border-indigo-500/30 text-indigo-400 rounded-2xl flex items-center justify-center shadow-[0_0_20px_rgba(79,70,229,0.15)] group-hover:scale-110 group-hover:bg-indigo-500 group-hover:text-white transition-all duration-500">
                      <Building2 className="h-7 w-7" />
                    </div>
                    <div>
                      <h3 className="font-outfit font-black text-xl text-white line-clamp-1 group-hover:text-indigo-300 transition-colors">{project.name}</h3>
                      <p className="text-xs font-medium text-indigo-200/60 uppercase tracking-widest mt-1">{project.clientName}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto relative z-10">
                  ...
                  <div className="flex justify-between items-center pt-6 border-t border-white/5">
                    <Link to={`/projects/${project.id}`} className="flex-1 mr-4">
                      <Button variant="outline" className="w-full justify-between border-white/10 hover:border-indigo-500/50 hover:bg-indigo-500/10 text-white rounded-xl transition-all duration-300 h-12 bg-black/20">
                        <span className="font-bold tracking-wide">Enter Matrix</span>
                        <ArrowRight className="h-4 w-4 ml-2 text-indigo-400 group-hover:translate-x-1 transition-transform" />
                      </Button>
                    </Link>
        ...
        {projects.length === 0 && !isCreating && (
          <motion.div variants={item} className="col-span-full">
            <div className="flex flex-col items-center justify-center py-24 px-4 glass-card border border-dashed border-white/20 relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-outfit font-black text-white tracking-tight relative z-10">Empty Portfolio</h3>
              <p className="mt-2 text-indigo-200/60 max-w-sm text-center relative z-10 font-medium">Initialize your first villa construction asset to begin financial tracking.</p>
              <Button onClick={() => setIsCreating(true)} className="mt-8 rounded-xl px-8 py-6 bg-indigo-600 hover:bg-indigo-500 text-white shadow-[0_0_20px_rgba(79,70,229,0.3)] relative z-10">
                <Plus className="h-5 w-5 mr-2" />
